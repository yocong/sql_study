...
SELECT
    ...
    tb_dept.dept_nm
FROM tb_emp, tb_dept
WHERE tb_emp.dept_cd = tb_dept.dept_cd -- 부서코드가 일치하는 것만 필터
; 
...
WHERE E.dept_cd = D.dept_cd -- 부서코드가 일치하는 것만 필터
; 
...
SELECT
    ...
    E.dept_cd,
    D.dept_nm
FROM tb_emp E JOIN tb_dept D -- JOIN만 쓰면 앞에 INNER 생략
ON E.dept_cd = D.dept_cd -- join 조건
WHERE E.addr LIKE '%용인%' -- 일반 조건
    AND E.emp_nm LIKE '김%'
;
...
-- 공통된 부분을 알아서 찾아주고 한번만 표기
SELECT 
    A.emp_no, A.emp_nm, dept_cd, B.dept_nm
--    *
FROM tb_emp A
NATURAL JOIN tb_dept B -- ON절을 알아서 찾아줌 (공통된 부분
;


-- 공통된 부분이 동일한 이름이 아니기 때문에 NATURAL JOIN 불가
SELECT 
    *
FROM test_a A
